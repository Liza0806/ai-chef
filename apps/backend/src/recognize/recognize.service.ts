...
export class RecognizeService {
  ...
  async recognize(buffer: Buffer) {
    ...
    // загружаем буфер как изображение
    const img = await loadImage(buffer);
    const canvas = createCanvas(img.width, img.height);
    const ctx = canvas.getContext('2d');
    ctx.drawImage(img, 0, 0, img.width, img.height);

    // делаем тензор из Canvas
    const input = tf.browser.fromPixels(canvas as unknown as HTMLCanvasElement);
//@ts-ignore
    const predictions = await this.model.detect(input);

    input.dispose(); // чистим память
    return predictions;
  }
}
